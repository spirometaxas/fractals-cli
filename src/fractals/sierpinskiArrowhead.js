...
class SierpinskiArrowhead {
    ...
    _sierpinski(n, size, board, pos, rotation) {
        if (n === 0) {
        ...
        if (rotation === Rotations.STANDARD) {
            this._sierpinski(n - 1, size - 1, board, { x: pos.x, y: pos.y }, Rotations.RIGHT);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 2), y: pos.y + this.getHeight(size - 1) }, Rotations.STANDARD);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 1), y: pos.y }, Rotations.LEFT);
        } else if (rotation == Rotations.LEFT) {
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 1), y: pos.y }, Rotations.STANDARD);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x, y: pos.y }, Rotations.LEFT);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 2), y: pos.y + this.getHeight(size - 1) }, Rotations.RIGHT);
        } else if (rotation === Rotations.RIGHT) {
            this._sierpinski(n - 1, size - 1, board, { x: pos.x, y: pos.y }, Rotations.STANDARD);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 1), y: pos.y }, Rotations.RIGHT);
            this._sierpinski(n - 1, size - 1, board, { x: pos.x + this.getWidth(size - 2), y: pos.y + this.getHeight(size - 1) }, Rotations.LEFT);
        }
    }
    ...
    create(n, config) {
        ...
        this._sierpinski(step, n, board, { x: 0, y: 0 }, rotation);
